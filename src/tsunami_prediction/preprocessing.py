# src/tsunami_prediction/preprocessing.py
...
def _save_info_tables(df: pd.DataFrame, name: str):
...
def _save_shape_summary(rows_cols: List[Tuple[str, int, int]]):
...
def _standardize_columns(df: pd.DataFrame, col_map: Dict[str, str] | None = None) -> pd.DataFrame:
...
def _clean_whitespace(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.select_dtypes(include="object").columns:
        ...
        df[c] = df[c].replace({"nan": np.nan, "none": np.nan, "None": np.nan, "NONE": np.nan, "": np.nan})
    return df
...
def _map_tsu_binary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    candidates = [
        "tsu", "tsunami", "tsu_flag", "tsunami_flag", "is_tsunami",
        "tsunami_event", "tsunami_(0/1)", "tsunami_(y/n)"
    ]
...
# ---------- dedup utility ----------
def _drop_and_log(df: pd.DataFrame, subset, dataset: str, label: str) -> pd.DataFrame:
...
def _drop_duplicates_smart(df: pd.DataFrame, dataset: str) -> pd.DataFrame:
    ...
    priority = ["year", "month", "day", "hr", "mn", "sec",
                "latitude", "longitude", "name", "location", "country"]
...
def prepare_tectonic(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _standardize_columns(df, col_map={
    ...
    df = _to_numeric(df, [
        ...
        "latitude", "longitude", "depth", "mag", "mw", "ms", "mb", "ml",
        "tsu"
    ])
    # Mo/Dy -> month/day
    ...
    wanted = [
        "id", "year", "month", "day", "hr", "mn", "sec",
        "country", "area", "region", "location",
        "latitude", "longitude",
        "depth", "mag",
        "tsu",
    ]
...
def prepare_volcanic(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _to_numeric(df, [
        "year", "mo", "dy",
        "latitude", "longitude", "elevation", "vei", "eq", "tsu"
    ])
    df = df.rename(columns={"mo": "month", "dy": "day"})
...
def encode_and_scale(
    ...
    if dataset == "tectonic":
        num_cols = [c for c in ["mag", "depth", "latitude", "longitude", "year", "month", "day"] if c in df.columns]
        cat_cols = [c for c in ["country", "region", "area", "location"] if c in df.columns]
    else:
        num_cols = [c for c in ["vei", "elevation", "latitude", "longitude", "year", "month", "day", "eq"] if c in df.columns]
    ...
    pd.DataFrame({"feature": out_cols}).to_csv(TAB / f"{dataset}_feature_names.csv", index=False)

    return X_df, pipe, num_cols, cat_cols


# ================== WRITE/REUSE HELPERS ==================
def _save_or_reuse_clean(df_t_clean: pd.DataFrame,
                         df_v_clean: pd.DataFrame,
                         overwrite: bool) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if out_t_clean.exists() and out_v_clean.exists() and not overwrite:
        print("[INFO] Clean CSVs already exist — reusing. Use --overwrite to regenerate.")
...
def _maybe_write_csv(df: pd.DataFrame, path: Path, overwrite: bool, label: str):
...
# ================== MAIN ==================
def main(overwrite: bool = False, normalize: bool = True):
    # cari file mentah (nama baru & fallback)
    raw_tec = next((p for p in [RAW / "tectonic.csv", RAW / "raw_tectonic_global.csv", RAW / "raw_tectonic.csv"] if p.exists()), None)
    raw_vul = next((p for p in [RAW / "volcanic.csv", RAW / "raw_volcanic_global.csv", RAW / "raw_volcanic.csv"] if p.exists()), None)
    ...
    # simpan/reuse clean
    df_t_clean, df_v_clean = _save_or_reuse_clean(df_t_clean0, df_v_clean0, overwrite=overwrite)
    ...
    df_t_prep, _, _, _ = encode_and_scale(df_t_clean, dataset="tectonic", normalize=normalize)
    df_v_prep, _, _, _ = encode_and_scale(df_v_clean, dataset="volcanic", normalize=normalize)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Preprocessing = Filtering + Cleaning + (Optional) Normalization+Encoding"
    )
    ap.add_argument("--overwrite", action="store_true", help="replace processed files if exist")
    ap.add_argument("--no-normalize", action="store_true", help="skip normalization & encoding, save only clean CSVs")
    args = ap.parse_args()
